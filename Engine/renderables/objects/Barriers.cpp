...
Barriers::Barriers() : model("../Assets/objects/barrier/Concrete-Barrier_04.obj") {
    ...
    // left transforms
    auto leftPos = glm::vec3(-103.0F, -6.0F, -100.0F);
    for (int i = 0; i < 10; i++) {
    ...
    auto backPos = glm::vec3(-100.0F, -6.0F, -103.0F);
    for (int i = 0; i < 10; i++) {
        glm::mat4 transform = Config::IDENTITY_MATRIX;
    ...
    auto rightPos = glm::vec3(103.0F, -6.0F, -100.0F);
    for (int i = 0; i < 10; i++) {
        glm::mat4 transform = Config::IDENTITY_MATRIX;
    ...
    auto frontPos = glm::vec3(-100.0F, -6.0F, 103.0F);
    for (int i = 0; i < 10; i++) {
        glm::mat4 transform = Config::IDENTITY_MATRIX;
