...
constexpr int CHUNK_RENDER_DISTANCE = 100;
constexpr int X_MAP_CHUNKS = 30;
constexpr int Y_MAP_CHUNKS = 30;
constexpr int CHUNK_HEIGHT = 35;
constexpr int CHUNK_WIDTH = 16;

constexpr float WATER_HEIGHT = 0.0F;



class Terrain {
public:
    explicit Terrain(int chunkRenderDistance = CHUNK_RENDER_DISTANCE, int xMapChunks = X_MAP_CHUNKS, int yMapChunks = Y_MAP_CHUNKS,
            int chunkHeight = CHUNK_HEIGHT, int chunkWidth = CHUNK_WIDTH) : chunkRenderDistance(chunkRenderDistance),
                                                                           xMapChunks(xMapChunks), yMapChunks(yMapChunks),
                                                                           chunkHeight(chunkHeight), chunkWidth(chunkWidth),
                                                                            originX(chunkWidth * xMapChunks / 2.0F - chunkWidth / 2.0F),
                                                                            originY(chunkHeight * yMapChunks / 2.0F - chunkHeight / 2.0F),
                                                                            mapChunks(static_cast<std::vector<unsigned int>::size_type>(xMapChunks * yMapChunks))
    {
        ...
        for (int y = 0; y < yMapChunks; y++) {
            for (int x = 0; x < xMapChunks; x++) {
                ...
                if (std::abs(gridPosX - x) <= chunkRenderDistance && (y - gridPosY) <= chunkRenderDistance) {
                    ...
                    model = translate(model, glm::vec3(-chunkWidth / 2.0F + (chunkWidth - 1) * x, -2.0F, -chunkHeight / 2.0F + (chunkHeight - 1.0F) * y));
...
private:
    ...
    [[nodiscard]] auto generateNoiseMap(const int offsetX, const int offsetY) const -> std::vector<float> {
        ...
        for (int y = 0; y < chunkHeight; y++) {
            for (int x = 0; x < chunkWidth; x++) {
                ...
                float noiseHeight = 0;
                for (int i = 0; i < octaves; i++) {
                    const float xSample = (x + offsetX * (chunkWidth-1))  / noiseScale * freq;
                    const float ySample = (y + offsetY * (chunkHeight-1)) / noiseScale * freq;
    ...
    [[nodiscard]] auto generateVertices(const std::vector<float> &noise_map) const -> std::vector<float> {
        ...
        for (int y = 0; y < chunkHeight + 1; y++)
            for (int x = 0; x < chunkWidth; x++) {
                ...
                const float easedNoise = std::pow(noise_map[x + y*chunkWidth] * 1.1, 3);
    ...
    static auto generateNormals(const std::vector<int> &indices, const std::vector<float> &vertices) -> std::vector<float> {
    ...
    void generateMapChunk(GLuint &VAO, int x, int y) {
        ...
        const std::vector<float> colors = generateBiome(vertices, x, y);
    ...
    struct terrainColor {
    terrainColor(const float _height, const glm::vec3 _color) {
        height = _height;
        color = _color;
    };
    float height;
...
[[nodiscard]] auto generateBiome(const std::vector<float> &vertices, int xOffset, int yOffset) const -> std::vector<float> {
    ...
    biomeColors.emplace_back(WATER_HEIGHT * 0.5, getColor(60,  95, 190));   // Deep water
    biomeColors.emplace_back(WATER_HEIGHT,        getColor(60, 100, 190));  // Shallow water
    biomeColors.emplace_back(0.15, getColor(210, 215, 130));                // Sand
    biomeColors.emplace_back(0.30, getColor( 95, 165,  30));                // Grass 1
    biomeColors.emplace_back(0.40, getColor( 65, 115,  20));                // Grass 2
    biomeColors.emplace_back(0.50, getColor( 90,  65,  60));                // Rock 1
    biomeColors.emplace_back(0.80, getColor( 75,  60,  55));                // Rock 2
    biomeColors.emplace_back(1.00, getColor(255, 255, 255));                // Snow


    // Determine which color to assign each vertex by its y-coord
    // Iterate through vertex y values
    for (int i = 1; i < vertices.size(); i += 3) {
        for (auto & biomeColor : biomeColors) {
            // NOTE: The max height of a vertex is "meshHeight"
            if (vertices[i] <= biomeColor.height * meshHeight) {
