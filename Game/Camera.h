...
public:
    ...
    explicit Camera(glm::vec3 position = glm::vec3(0.0f, 0.0f, 0.0f), glm::vec3 up = glm::vec3(0.0f, 1.0f, 0.0f), float yaw = YAW, float pitch = PITCH, float damping = DAMPING, float smoothing = SMOOTHING, float acceleration = ACCELERATION, float maxspeed = MAXSPEED) : position(position),  worldUp(up), yaw(yaw), pitch(pitch), damping(damping), smoothing(smoothing), acceleration(acceleration), maxSpeed(maxspeed) {
